...
namespace Risk_World_Conquest
{
    class SlidingBackground
    {
        ...
        public SlidingBackground(ContentManager manager, string assetName)
        {
            ...
            lastCameraPosition = Camara.GetTarget();
            position = Camara.GetTarget();
            size = new Vector2(0.3f * Camara.worldWidth,
                0.3f * Camara.worldWidth * texture.Height / texture.Width);
        }
        ...
        {
            Vector2 movement = lastCameraPosition - Camara.GetTarget();
            ...
            Rectangle dest = Camara.WorldSize2PixelRectangle(position, size); // recebe a posição onde queriamos desenhar a sprite, position centro da camara

            xMin = -(int)Math.Ceiling((dest.X - 0.5f * dest.Width) / dest.Width);
            yMin = -(int)Math.Ceiling((dest.Y - 0.5f * dest.Height) / dest.Height);

            xMax = (int)Math.Ceiling((Camara.gDevManager.PreferredBackBufferWidth - dest.X - dest.Width * .5f) / dest.Width);
            yMax = (int)Math.Ceiling((Camara.gDevManager.PreferredBackBufferWidth - dest.Y - dest.Height * .5f) / dest.Height);
